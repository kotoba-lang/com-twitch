(ns twitch.main
  "Kotodama WASM entrypoint for the Twitch clean-room actor (L4) — Clojure port.
  ...
  facts (`twitch.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "twitch")
(def tier "L4")
...
(def entity-specs
  [{:entity "User"    :plural "users"    :id-prefix "twitch_use"
    :fields [:userLogin :userName :broadcasterType] :required [:userLogin :userName]
    :coerce {}                       :refs {}}
   {:entity "Channel"  :plural "channels" :id-prefix "twitch_cha"
    :fields [:broadcasterLogin :broadcasterName :broadcasterLanguage :gameId :gameName :title :delay] :required [:broadcasterLogin :broadcasterName]
    :coerce {:delay :int}            :refs {:gameId "Game"}}
   {:entity "Stream"   :plural "streams"  :id-prefix "twitch_str"
    :fields [:userId :userName :gameId :gameName :type :viewerCount :title :language] :required [:userName :gameName]
    :coerce {:viewerCount :int}      :refs {:userId "User" :gameId "Game"}}
   {:entity "Video"    :plural "videos"   :id-prefix "twitch_vid"
    :fields [:userId :title :description :type :viewable :viewCount :duration] :required [:title :description]
    :coerce {:viewCount :int :duration :int} :refs {:userId "User"}}
   {:entity "Clip"     :plural "clips"    :id-prefix "twitch_cli"
    :fields [:broadcasterId :creatorId :gameId :title :viewCount :duration :isFeatured] :required [:title :viewCount]
    :coerce {:viewCount :int :duration :float :isFeatured :bool} :refs {:gameId "Game"}}
   {:entity "Game"     :plural "games"    :id-prefix "twitch_gam"
    :fields [:name :boxArtUrl]             :required [:name :boxArtUrl]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"twitch.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "twitch-compat" :tier tier :entities entities} 200])
...
(defn start! [] :twitch-compat/ready)
